...
The Frontend component is tasked with: (1) Provide a User Interface (UI), on a webpage, for viewing sensor data. (2) Provide a user interface (UI), on a webpage, featuring charts and graphs to display real-time status of the Water Management System. (3) Another webpage, which should integrate a visual representation, such as a chart or graph, extracting trends and illustrating any correlation among variables, from data stored in the database.  
