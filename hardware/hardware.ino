...
void setup() {
  ...
  tft.setRotation(2); // Adjust if necessary for proper orientation

  // Show UWI Logo (Ensure it's correctly loaded in your code)
  tft.drawRGBBitmap(0, 0, UWI, 240, 320);  // Adjust x, y position if needed

  // Display Text Below the Logo (Centered)
  tft.setTextColor(ILI9341_RED, ILI9341_WHITE);  // Red text on white background
  tft.setTextSize(2);
  
  //int textX = (240 - (12 * 12)) / 2;  // Rough centering calculation
  //tft.setCursor(textX, 210);  
  //tft.println("Home Wheather System");

  // Pause for 3 seconds before switching to sensor display
  delay(2000);

  // Clear Screen and Proceed to Normal Sensor Display
